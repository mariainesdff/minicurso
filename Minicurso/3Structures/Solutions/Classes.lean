...
instance : AddGroup R2 where
  ...
  add_assoc p q r := by
    ext
    . simp only [R2.x, add_assoc]
    . simp only [R2.y, add_assoc]
  zero := 0
  zero_add p := by
    ext
    . simp only [R2.x, zero_add]
    . simp only [R2.y, zero_add]
  add_zero p := by
    ext
    . simp only [R2.x, add_zero]
    . simp only [R2.y, add_zero]
  neg p := -p
  add_left_neg p := by
    ext
    . simp only [R2.x, add_left_neg]
    . simp only [R2.y, add_left_neg]
  nsmul   := nsmulRec
  zsmul := zsmulRec
