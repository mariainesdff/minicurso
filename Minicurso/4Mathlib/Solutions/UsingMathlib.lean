...
example {α : Type*} (s t u : Set α) (h : s ⊆ t) : s ∩ u ⊆ t ∩ u := by
  exact Set.inter_subset_inter h fun ⦃a⦄ a => a -- foun using by exact?

example {α : Type*} (s t u : Set α) : s ∩ (t ∪ u) ⊆ s ∩ t ∪ s ∩ u := by
  intro x hx
  rw [@Set.mem_union] -- found by rw?
  rw [@Set.mem_inter_iff] at hx -- found by rw? at hx
  rcases hx with ⟨hs, (ht | hu)⟩
  · left
    exact Set.mem_inter hs ht -- found by apply? or exact?
  · right
    exact Set.mem_inter hs hu -- found by apply? or exact?

example {a b : ℝ} : 2 * a * b ≤ a ^ 2 + b ^ 2 := by
  have h : 0 ≤ a ^ 2 - 2 * a * b + b ^ 2 := by
    calc
      a ^ 2 - 2 * a * b + b ^ 2 = (a - b) ^ 2 := by ring
      _ ≥ 0 := by exact sq_nonneg (a - b) -- Found using by exact?
  linarith
...
/- Search for [`​First isomorphism theorem for commutative rings`](https://www.moogle.ai/search/raw?q=First%20isomorphism%20theorem%20for%20commutative%20rings) -/
...
example {α β : Type*} [TopologicalSpace α] [Preorder α] [TopologicalSpace β] [t : OrderTopology α]
    ...
    Filter.Tendsto f (nhds b) (nhds a) :=
  tendsto_of_tendsto_of_tendsto_of_le_of_le hg hh hgf hfh

/- Search for [`​Sandwich lemma`](https://www.moogle.ai/search/raw?q=Sandwich%20lemma) -/
